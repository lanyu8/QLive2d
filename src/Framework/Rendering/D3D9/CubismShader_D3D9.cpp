    ...
    namespace Cubism
    {
        namespace Framework
        {
            namespace Rendering
            {
                ...
                CubismShader_D3D9::CubismShader_D3D9() : _shaderEffect(NULL), _vertexFormat(NULL)
                {
                }
                ...
                void CubismShader_D3D9::GenerateShaders(LPDIRECT3DDEVICE9 device)
                {
                    ...
                    // この描画で使用する頂点フォーマット
                    D3DVERTEXELEMENT9 elems[] = { { 0, 0, D3DDECLTYPE_FLOAT2, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_POSITION, 0 },
                ...
                Csm::csmBool CubismShader_D3D9::LoadShaderProgram(LPDIRECT3DDEVICE9 device)
                {
                    ...
                    if (FAILED(D3DXCreateEffect(device, CubismShaderEffectSrc, static_cast<UINT>(strlen(CubismShaderEffectSrc)), 0, 0, 0, 0,
                                                &_shaderEffect, &error)))
                    {
                        return false;
                    }
