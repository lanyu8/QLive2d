            ...
            namespace Rendering
            {
                ...
                void CubismRenderState_D3D9::Restore(LPDIRECT3DDEVICE9 device)
                {
                    ...
                    for (csmInt32 i = static_cast<csmInt32>(_pushed.GetSize()) - 1; i >= 0; i--)
                    {
                        ...
                        {
                            SetBlend(device, current.BlendEnable, current.BlendAlphaSeparateEnable, current.BlendSourceMul,
                                     current.BlendBlendFunc, current.BlendDestMul, current.BlendSourceAlpha, current.BlendAlphaFunc,
                                     current.BlendDestAlpha, true);
                            isSet[State_Blend] = true;
                        }
                        if (_pushed[i]._valid[State_Viewport] && !isSet[State_Viewport])
                        {
                            SetViewport(device, current.ViewportX, current.ViewportY, current.ViewportWidth, current.ViewportHeight,
                                        current.ViewportMinZ, current.ViewportMaxZ, true);
                        ...
                        {
                            SetTextureFilter(device, current.MinFilter, current.MagFilter, current.MipFilter, current.AddressU, current.AddressV,
                                             true);
                ...
                void CubismRenderState_D3D9::SetBlend(LPDIRECT3DDEVICE9 device, bool enable, bool alphaSeparateEnable, D3DBLEND srcmul,
                                                      D3DBLENDOP blendFunc, D3DBLEND destmul, D3DBLEND srcalpha, D3DBLENDOP alphaFunc,
                                                      D3DBLEND destalpha, csmBool force)
                {
                    if (!_stored._valid[State_Blend] || force || _stored.BlendEnable != enable ||
                        _stored.BlendAlphaSeparateEnable != alphaSeparateEnable || _stored.BlendSourceMul != srcmul ||
                        _stored.BlendBlendFunc != blendFunc || _stored.BlendDestMul != destmul || _stored.BlendSourceAlpha != srcalpha ||
                        _stored.BlendAlphaFunc != alphaFunc || _stored.BlendDestAlpha != destalpha)
                    {
                ...
                void CubismRenderState_D3D9::SetViewport(LPDIRECT3DDEVICE9 device, DWORD left, DWORD top, DWORD width, DWORD height, float zMin,
                                                         float zMax, csmBool force)
                {
                    if (!_stored._valid[State_Blend] || force || _stored.ViewportX != left || _stored.ViewportY != top ||
                        _stored.ViewportWidth != width || _stored.ViewportHeight != height || _stored.ViewportMinZ != zMin ||
                ...
                {
                    if (!_stored._valid[State_Blend] || force || _stored.ColorMaskMask != mask)
                    {
                ...
                {
                    if (!_stored._valid[State_ZEnable] || force || _stored.ZEnable != enable || _stored.ZFunc != zfunc)
                    {
                ...
                {
                    if (!_stored._valid[State_CullMode] || force || _stored.CullModeFaceMode != cullFace)
                    {
                ...
                void CubismRenderState_D3D9::SetTextureFilter(LPDIRECT3DDEVICE9 device, D3DTEXTUREFILTERTYPE minFilter,
                                                              D3DTEXTUREFILTERTYPE magFilter, D3DTEXTUREFILTERTYPE mipFilter,
                                                              D3DTEXTUREADDRESS addressU, D3DTEXTUREADDRESS addressV, csmBool force)
                {
                    if (!_stored._valid[State_TextureFilter] || force || _stored.MinFilter != minFilter || _stored.MagFilter != magFilter ||
                        _stored.MipFilter != mipFilter || _stored.AddressU != addressU || _stored.AddressV != addressV)
                    {
                ...
                void CubismRenderState_D3D9::SaveCurrentNativeState(LPDIRECT3DDEVICE9 device)
                {
                    ...
                    SetBlend(device, (setting[0] ? true : false), (setting[1] ? true : false), static_cast<D3DBLEND>(setting[2]),
                             static_cast<D3DBLENDOP>(setting[3]), static_cast<D3DBLEND>(setting[4]), static_cast<D3DBLEND>(setting[5]),
                             static_cast<D3DBLENDOP>(setting[6]), static_cast<D3DBLEND>(setting[7]), true);
                    ...
                    SetTextureFilter(device, static_cast<D3DTEXTUREFILTERTYPE>(setting[0]), static_cast<D3DTEXTUREFILTERTYPE>(setting[1]),
                                     static_cast<D3DTEXTUREFILTERTYPE>(setting[2]), static_cast<D3DTEXTUREADDRESS>(setting[3]),
                                     static_cast<D3DTEXTUREADDRESS>(setting[4]), true);
