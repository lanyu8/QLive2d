    ...
    namespace Cubism
    {
        namespace Framework
        {
            namespace Rendering
            {
                ...
                CubismShader_D3D11::CubismShader_D3D11() : _vertexFormat(NULL)
                {
                    // 器作成
                    for (csmInt32 i = 0; i < ShaderNames_Max; i++)
                    {
                ...
                void CubismShader_D3D11::GenerateShaders(ID3D11Device *device)
                {
                    ...
                    do
                    {
                        ...
                        if (!LoadShaderProgram(device, true, ShaderNames_NormalMaskedPremultipliedAlpha,
                                               static_cast<const csmChar *>("PixelMaskedPremult")))
                        {
                            break;
                        }
                        if (!LoadShaderProgram(device, true, ShaderNames_NormalMaskedInvertedPremultipliedAlpha,
                                               static_cast<csmChar *>("PixelMaskedInvertedPremult")))
                        {
                            break;
                        }
                    ...
                    HRESULT hr = D3DCompile(CubismShaderEffectSrc,         // メモリー内のシェーダーへのポインターです。
                    ...
                    else
                    {
                        ...
                        hr = device->CreateInputLayout(elems, ARRAYSIZE(elems), layoutBlobr->GetBufferPointer(), layoutBlobr->GetBufferSize(),
                                                       &_vertexFormat);
                ...
                Csm::csmBool CubismShader_D3D11::LoadShaderProgram(ID3D11Device *device, bool isPs, csmInt32 assign, const csmChar *entryPoint)
                {
                    ...
                    do
                    {
                        ...
                        hr = D3DCompile(CubismShaderEffectSrc,         // メモリー内のシェーダーへのポインターです。
                                        strlen(CubismShaderEffectSrc), // メモリー内のシェーダーのサイズです。
                                        NULL, // シェーダー コードが格納されているファイルの名前です。
                                        NULL, // マクロ定義の配列へのポインターです
                                        NULL, // インクルード ファイルを扱うインターフェイスへのポインターです
