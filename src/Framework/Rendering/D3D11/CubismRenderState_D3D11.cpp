            ...
            namespace Rendering
            {
                ...
                void CubismRenderState_D3D11::Restore(ID3D11DeviceContext *renderContext)
                {
                    ...
                    for (csmInt32 i = static_cast<csmInt32>(_pushed.GetSize()) - 1; i >= 0; i--)
                    {
                        ...
                        {
                            SetViewport(renderContext, current._viewportX, current._viewportY, current._viewportWidth, current._viewportHeight,
                                        current._viewportMinZ, current._viewportMaxZ, true);
                ...
                void CubismRenderState_D3D11::SetBlend(ID3D11DeviceContext *renderContext, Blend blendState, DirectX::XMFLOAT4 blendFactor,
                                                       UINT mask, csmBool force)
                {
                    ...
                    if (!_stored._valid[State_Blend] || force || _stored._blendFactor.x != blendFactor.x ||
                        _stored._blendFactor.y != blendFactor.y || _stored._blendFactor.z != blendFactor.z ||
                        _stored._blendFactor.w != blendFactor.w || _stored._blendMask != mask || _stored._blendState != blendState)
                    {
                ...
                void CubismRenderState_D3D11::SetCullMode(ID3D11DeviceContext *renderContext, Cull cullFace, csmBool force)
                {
                    ...
                    if (!_stored._valid[State_CullMode] || force || _stored._cullMode != cullFace)
                    {
                ...
                void CubismRenderState_D3D11::SetViewport(ID3D11DeviceContext *renderContext, FLOAT left, FLOAT top, FLOAT width, FLOAT height,
                                                          FLOAT zMin, FLOAT zMax, csmBool force)
                {
                    if (!_stored._valid[State_Blend] || force || _stored._viewportX != left || _stored._viewportY != top ||
                        _stored._viewportWidth != width || _stored._viewportHeight != height || _stored._viewportMinZ != zMin ||
                ...
                void CubismRenderState_D3D11::SetZEnable(ID3D11DeviceContext *renderContext, Depth enable, UINT stelcilRef, csmBool force)
                {
                    ...
                    if (!_stored._valid[State_ZEnable] || force || _stored._depthEnable != enable)
                    {
                ...
                void CubismRenderState_D3D11::SetSampler(ID3D11DeviceContext *renderContext, Sampler sample, csmBool force)
                {
                    ...
                    if (!_stored._valid[State_ZEnable] || force || _stored._sampler != sample)
                    {
                        // 0番だけ使用している
                ...
                void CubismRenderState_D3D11::SaveCurrentNativeState(ID3D11Device *device, ID3D11DeviceContext *renderContext)
                {
                    ...
                    SetViewport(renderContext, viewPort.TopLeftX, viewPort.TopLeftY, viewPort.Width, viewPort.Height, viewPort.MinDepth,
                                viewPort.MaxDepth, true);
