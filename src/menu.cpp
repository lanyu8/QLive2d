...
MenuItem::MenuItem(QPixmap pm) : QLabel(nullptr)
{
    ...
    connect(group1, &QParallelAnimationGroup::stateChanged, group1, [this](QParallelAnimationGroup::State s1, QParallelAnimationGroup::State) {
...
MenuItem::MenuItem(QPixmap p1, QPixmap p2) : MenuItem(p1)
{
    this->pix1 = p1;
    this->pix2 = p2;
}
...
Menu::Menu()
{
    items[0] = new MenuItem(QPixmap::fromImage(QImage(":/icons/music.png").scaled(30, 30)),
                            QPixmap::fromImage(QImage(":/icons/nomusic.png").scaled(30, 30)));
...
{
    for (int i = 0; i < 4; i++) items[i]->moveto(pt + QPoint(0, i * 50));
}
